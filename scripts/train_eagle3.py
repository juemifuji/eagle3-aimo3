...
def parse_args() -> Tuple[ArgumentParser, Namespace]:
    ...
    # === NEW: distillation/memory knobs ===
    training_group.add_argument(
        "--distill-mode",
        type=str,
        default="hard",
        choices=["hard", "soft"],
        help="hard: teacher provides only top1 token ids; soft: full prob dist (very memory heavy).",
    )
    training_group.add_argument(
        "--logits-chunk-size",
        type=int,
        default=256,
        help="Chunk size for computing draft logits/loss. Smaller uses less VRAM.",
    )
    training_group.add_argument(
        "--offload-eagle3-data-to-cpu",
        action="store_true",
        help="(Not recommended) Offload eagle3 target outputs to CPU after generation.",
    )
    training_group.add_argument(
        "--offload-threshold-mb",
        type=int,
        default=256,
        help="If offload enabled, tensors larger than this will be moved to CPU.",
    )
    ...
    # DeepSpeed / torchrun compatibility
    parser.add_argument(
        "--local_rank",
        type=int,
        default=int(os.environ.get("LOCAL_RANK", 0)),
        help="Local rank passed by launcher (deepspeed/torchrun).",
    )
...
def sanity_check(args: Namespace) -> None:
    ...
    # IMPORTANT (OOM fix):
    # Target model already runs in tensor-parallel across tp_size ranks.
    # Multiplying batch by tp_size forces every TP rank to redundantly compute
    # tp_size times more samples, and explodes memory at TP vocab all-gather.
...
def build_draft_model(args: Namespace) -> Tuple[AutoDraftModelConfig, nn.Module]:
    ...
    if type(draft_model_config) == str:
...
def save_checkpoints(
    ...
    with FSDP.state_dict_type(eagle3_model, StateDictType.FULL_STATE_DICT):
        ...
        state_to_save = {"epoch": epoch, "global_step": step, "args": args}
...
def _move_large_cuda_tensors_to_cpu(
    obj,
    threshold_bytes=256 * 1024 * 1024,
    cast_dtype=torch.bfloat16,
):
    """Move very large CUDA tensors in obj to CPU (in-place)."""
    def handle_value(name, v):
        if torch.is_tensor(v) and v.is_cuda:
            nbytes = v.numel() * v.element_size()
            if nbytes >= threshold_bytes:
                nv = v.detach()
                if cast_dtype is not None and nv.dtype in (
                    torch.float16,
                    torch.bfloat16,
                    torch.float32,
                ):
                    nv = nv.to(dtype=cast_dtype)
                nv = nv.cpu()
                return nv, True, nbytes
        return v, False, 0
...
def _truncate_1d_batch(x: torch.Tensor, max_len: int) -> torch.Tensor:
    """Truncate [B,T] or [B,T,1] to max_len along T."""
    if x is None or not torch.is_tensor(x):
        return x
    if x.dim() == 2 and x.size(1) > max_len:
        return x[:, :max_len]
    if x.dim() == 3 and x.size(1) > max_len:
        return x[:, :max_len, :]
    return x
...
def run_forward(
    ...
    if args.is_vlm:
        # (Optional) truncate text-side tensors to avoid pathological sequences
        input_ids = _truncate_1d_batch(data["input_ids"].cuda(), args.max_length)
        attention_mask = _truncate_1d_batch(data["attention_mask"].cuda(), args.max_length)
        loss_mask = _truncate_1d_batch(data["loss_mask"].cuda(), args.max_length)
        ...
        if is_online:
            # 0) move batch to cuda + FORCE TRUNCATE to args.max_length
            input_ids_gpu = _truncate_1d_batch(data["input_ids"].cuda(), args.max_length)
            attention_mask_gpu = _truncate_1d_batch(data["attention_mask"].cuda(), args.max_length)
            loss_mask_gpu = _truncate_1d_batch(data["loss_mask"].cuda(), args.max_length)

            # 1) generate eagle3 target outputs
            eagle3_data = target_model.generate_eagle3_data(
                input_ids=input_ids_gpu,
                attention_mask=attention_mask_gpu,
                loss_mask=loss_mask_gpu,
            )

            # 2) hard distill: immediately convert teacher logits -> tokens to free VRAM
            #    (If eagle3_target_model.py already returns tokens, this is a no-op.)
            if args.distill_mode == "hard" and torch.is_tensor(eagle3_data.target) and eagle3_data.target is not None:
                if eagle3_data.target.dim() == 3:
                    with torch.no_grad():
                        eagle3_data.target = eagle3_data.target.argmax(-1).to(torch.long)
                    torch.cuda.empty_cache()

            # 3) OPTIONAL: offload to CPU (default OFF)
            if args.offload_eagle3_data_to_cpu:
                _move_large_cuda_tensors_to_cpu(
                    eagle3_data,
                    threshold_bytes=args.offload_threshold_mb * 1024 * 1024,
                    cast_dtype=torch.bfloat16,
                )
                torch.cuda.empty_cache()
            ...
            # 5) if offloaded, bring back to cuda explicitly
            if args.offload_eagle3_data_to_cpu:
                if torch.is_tensor(hidden_states) and not hidden_states.is_cuda:
                    hidden_states = hidden_states.cuda(non_blocking=True)
                if torch.is_tensor(target) and not target.is_cuda:
                    target = target.cuda(non_blocking=True)
                if torch.is_tensor(loss_mask) and not loss_mask.is_cuda:
                    loss_mask = loss_mask.cuda(non_blocking=True)
                if torch.is_tensor(input_ids) and not input_ids.is_cuda:
                    input_ids = input_ids.cuda(non_blocking=True)
                if torch.is_tensor(attention_mask) and not attention_mask.is_cuda:
            ...
            input_ids, target, loss_mask = target_model.preprocess(input_ids, target, loss_mask)
...
def run_backward_and_update(
    ...
    ploss = sum(ploss_weight[i] * plosses[i] for i in range(len(plosses))) / args.draft_accumulation_steps
    ploss.backward()
...
def get_dp_data_shard_from_tp(tensor: torch.Tensor) -> torch.Tensor:
    ...
    if tensor.size(0) < tp_size or tensor.size(0) % tp_size != 0:
        return tensor
...
def _safe_init(cls, **kwargs):
    """Init class with kwargs filtered by signature for backwards compatibility."""
...
def main():
    ...
    # === bind CUDA device explicitly to avoid NCCL "Guessing device ID" ===
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if dist.get_rank() == 0:
            progress_bar = tqdm(train_dataloader, desc=f"Training Epoch {epoch}", leave=True)
        else:
            ...
            if args.eval_data_path is not None and global_step % args.eval_interval == 0:
                ...
                for data in tqdm(eval_dataloader, desc=f"Evaluating Epoch {epoch}"):
                    with torch.no_grad():
                        ...
                        eval_plosses = [eval_plosses[i] + [plosses[i]] for i in range(len(plosses))]
